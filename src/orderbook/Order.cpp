...
Order::Order(std::string _client, Price _price, int _order_id, int _volume, Side _side, std::time_t _timestamp):
	client_{_client},
	price_{_price},
	order_id_{_order_id},
	volume_{_volume},
...
Order::Order(std::string _client, double _price, int _order_id, int _volume, Side _side, std::time_t _timestamp):
    client_(_client),
    price_(Price(_price)),
    order_id_(_order_id),
    volume_(_volume),
...
std::time_t Order::get_timestamp() const { return timestamp_; }
...
void Order::set_timestamp(std::time_t new_timestamp) { timestamp_ = new_timestamp; }
